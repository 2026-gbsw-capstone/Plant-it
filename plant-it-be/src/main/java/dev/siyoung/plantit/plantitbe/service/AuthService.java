...
import dev.siyoung.plantit.plantitbe.repository.EmailVerificationTokenRepository;
...
public class AuthService {
    ...
    public EmailVerificationRequestResponseDto requestSignupEmailVerification(EmailVerificationRequestDto request) {
        if (userRepository.existsByEmail(request.getEmail())) {
            throw new PlantItException(HttpStatus.CONFLICT, "이미 가입된 이메일입니다.");
        }

        // 새 코드를 발급하기 전에 이전에 발급된 미사용 코드를 모두 무효화한다.
        emailVerificationTokenRepository.invalidateActiveTokensByEmail(request.getEmail());
        ...
        emailVerificationTokenRepository.save(EmailVerificationToken.builder()
                ...
                .expiresAt(expiresAt)
                .build());
        emailService.sendSignupVerificationCode(request.getEmail(), code, expiresAt);

        return EmailVerificationRequestResponseDto.builder()
                .email(request.getEmail())
                .expiresAt(expiresAt)
                .build();
    }

    public void verifySignupEmail(EmailVerificationConfirmRequestDto request) {
    ...
    public SignupResponseDto signup(SignupRequestDto request) {
        ...
        // 이메일 인증을 완료한(verified) 미사용 코드가 있어야 회원가입을 허용한다.
        EmailVerificationToken verification = emailVerificationTokenRepository
                ...
                .orElseThrow(() -> new PlantItException(HttpStatus.BAD_REQUEST, "이메일 인증이 필요합니다."));
        if (verification.getExpiresAt().isBefore(LocalDateTime.now())) {
            throw new PlantItException(HttpStatus.BAD_REQUEST, "이메일 인증이 만료되었습니다. 다시 인증해주세요.");
        }
        ...
        User user = User.builder()
                ...
                .build();
        SignupResponseDto response = EntityDtoMapper.toSignupDto(userRepository.save(user));
    ...
    public PasswordResetRequestResponseDto requestPasswordReset(PasswordResetRequestDto request) {
        ...
        // 새 코드를 발급하기 전에 이전에 발급된 미사용 코드를 모두 무효화한다.
        passwordResetTokenRepository.invalidateActiveTokensByEmail(user.getEmail());
